...
(function () {
  ...
  function mkRenderer(dark) {
   var P = dark
     ? { band: 'rgba(255,255,255,.028)', grid: 'rgba(255,255,255,.09)', gtext: '#9a8c78', line: '#df7d54', lane: 'rgba(255,255,255,.16)', now: 'rgba(223,125,84,.6)', ring: '#221b14', a0: '.26', doseL: 65, doseF: 66 }
     : { band: 'rgba(43,33,24,.025)', grid: 'rgba(43,33,24,.09)', gtext: '#9a8c78', line: '#bf5b39', lane: 'rgba(43,33,24,.16)', now: 'rgba(191,91,57,.5)', ring: '#fbf6ec', a0: '.2', doseL: 52, doseF: 55 };
   return function (v) {
    var b = v.bands, pts = v.painPts, s = '';
    s += '<defs><linearGradient id="apArea' + (dark ? 'D' : '') + '" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="' + P.line + '" stop-opacity="' + P.a0 + '"/><stop offset="1" stop-color="#cf8b3c" stop-opacity="0"/></linearGradient></defs>';
    v.dayTicks.forEach(function (t, i) { if (i % 2 === 1) { var x2 = v.x(t.t + D.DAY), x1 = Math.max(0, t.x); s += '<rect x="' + x1.toFixed(1) + '" y="' + b.painTop + '" width="' + Math.max(0, Math.min(v.W, x2) - x1).toFixed(1) + '" height="' + (b.laneBot - b.painTop).toFixed(1) + '" fill="' + P.band + '"/>'; } });
    [[10, b.painTop], [5, (b.painTop + b.painBot) / 2], [0, b.painBot]].forEach(function (g) {
      s += '<line x1="24" y1="' + g[1].toFixed(1) + '" x2="' + (v.W - 12) + '" y2="' + g[1].toFixed(1) + '" stroke="' + P.grid + '"/>' +
        '<text x="16" y="' + (g[1] + 3).toFixed(1) + '" font-size="9" fill="' + P.gtext + '" text-anchor="end" font-family="Figtree, sans-serif">' + g[0] + '</text>';
    });
    var line = smooth(pts);
    if (line && pts.length) s += '<path d="' + line + ' L' + pts[pts.length - 1].x.toFixed(1) + ' ' + b.painBot + ' L' + pts[0].x.toFixed(1) + ' ' + b.painBot + ' Z" fill="url(#apArea' + (dark ? 'D' : '') + ')"/>';
    if (line) s += '<path d="' + line + '" fill="none" stroke="' + P.line + '" stroke-width="2.4" stroke-linecap="round"/>';
    pts.forEach(function (p) { var r = v.detail ? 4 : 2.8; s += '<circle cx="' + p.x.toFixed(1) + '" cy="' + p.y.toFixed(1) + '" r="' + r + '" fill="' + P.line + '"' + (p.note ? ' stroke="' + P.ring + '" stroke-width="2"' : '') + '/>'; });
    s += '<text x="24" y="' + (b.laneTop - 3).toFixed(1) + '" font-size="8.5" fill="' + P.gtext + '" letter-spacing=".1em" font-family="Figtree, sans-serif">DOSES</text>';
    s += '<line x1="24" y1="' + b.laneBot.toFixed(1) + '" x2="' + (v.W - 12) + '" y2="' + b.laneBot.toFixed(1) + '" stroke="' + P.lane + '"/>';
    v.doseItems.forEach(function (d) {
      var hh = 8 + Math.min(1, d.units / 2) * ((b.laneBot - b.laneTop) * 0.7), cy = b.laneBot - hh, w = d.units >= 2 ? 8 : 6;
      s += '<line x1="' + d.x.toFixed(1) + '" y1="' + b.laneBot.toFixed(1) + '" x2="' + d.x.toFixed(1) + '" y2="' + cy.toFixed(1) + '" stroke="hsl(' + d.hue + ' 55% ' + P.doseL + '% / .5)" stroke-width="2"/>' +
        '<rect x="' + (d.x - w / 2).toFixed(1) + '" y="' + (cy - 5).toFixed(1) + '" width="' + w.toFixed(1) + '" height="10" rx="5" fill="hsl(' + d.hue + ' 58% ' + P.doseF + '%)"/>';
    });
    s += '<line x1="' + v.nowX.toFixed(1) + '" y1="' + b.painTop + '" x2="' + v.nowX.toFixed(1) + '" y2="' + b.laneBot.toFixed(1) + '" stroke="' + P.now + '" stroke-dasharray="2 3"/>';
    v.axisTicks.forEach(function (t) { s += '<text x="' + t.x.toFixed(1) + '" y="' + b.axisY.toFixed(1) + '" font-size="9.5" fill="' + P.gtext + '" text-anchor="middle" font-family="Figtree, sans-serif">' + t.label + '</text>'; });
    return s;
   };
  }
  ...
  var base = {
    tagline: 'Warm, tactile — your daily doses as a blister pack.',
    ...
    span: 4 * D.DAY, renderApp: renderApp, renderSheet: renderSheet
  };
  window.CONCEPTS.apothecary = Object.assign({}, base, {
    label: 'Apothecary', meta: { type: 'Spectral serif · Figtree', palette: 'Warm cream · amber · clay', idea: 'Doses as a tactile blister pack' },
    timelineRenderer: mkRenderer(false)
  });
  window.CONCEPTS['apothecary-dark'] = Object.assign({}, base, {
    label: 'Apothecary Dark', tagline: 'Apothecary by candlelight — warm amber on espresso.',
    meta: { type: 'Spectral serif · Figtree', palette: 'Espresso · amber · clay', idea: 'Doses as a tactile blister pack' },
